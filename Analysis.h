...
{					   //Função para comparar strings, baseado em s_1
	bool equal = true; //começa como iguais
	for (int i = 0; i < s_1.length(); i++)
	{ //No tamanho de s_1
		if (s_1[i] != s_2[i])
		{ //Compara pra ver se existe alguma letra diferente
			equal = false;
		}
	}
	return equal;
...
class Analysis
{ //Modulado em uma classe de analise
...
private:
	...
	)
	{
		...
		for (int a = 0; a < N; a++)
		{
			...
			if (a != 0)
			{ //Coloca os elementos utilizados como comparadores
		...
		for (int j = 0; j < size; j++)
		{
			...
			{
				if (k < cMatrix[j].size())
				{												//Tendo o elemento como existente
	...
	)
	{
		...
		{ //Se o nó não for nulo
			...
			for (int i = 0; i < N; i++)
			{ //Para a quantidade de comparadores
				if (!Compare(comparators[i], (node->getData().*comparatorTypes[i])()))
				{					 //Utiliza função Compare para ver se o elemento (no campo dado) começa com a string dada em comparators[i]
			...
			if (!filtered)
			{											  //Caso o elemento não tenha sido filtrado
		...
		Data *cResults)
	{
		...
		if (node != NULL)
		{
			...
			{														  //Se o elemento está em uma posição desejada de listagem
	...
	)
	{
		...
		if (node != NULL)
		{
			...
			for (int i = 0; i < *N; i++)
			{ //Para a contagem de comparadores já existentes
...
public:
	Analysis(Tree a)
	{ //Construtor só requerendo a arvore para analise
		arv = a;
	}
	...
	void Fragment(int begin = 0, int end = -1, int step = 1, string (Data::*infoType)() = &Data::getTitle)
	{
		...
		if (step != 1)
		{ //não precisa setar para operação com módulo, caso o step seja 1
	...
	void List(string (Data::*comparatorType)(), string comparator = "", string (Data::*infoType)() = &Data::getTitle)
	{
		...
		if (comparator == "")
		{						  //Se a string opcional de filtro for vazia, por tal indiciando que não se deseja filtrar os elementos
